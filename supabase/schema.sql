...
create policy "Public read user keys" on user_keys
  for select
  using (true);
create policy "Users can insert own key" on user_keys
  for insert
  with check (user_id = auth.uid());
create policy "Users can update own key" on user_keys
  for update
  using (user_id = auth.uid());
